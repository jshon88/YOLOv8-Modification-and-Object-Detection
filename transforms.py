...
class Squeeze:
    def __call__(self, image: torch.Tensor, target: Dict[str, Any]):
        ...
        Returns:
            Tuple[torch.Tensor, Dict[str, Any]]: Squeezed image and unchanged target.
        """
        if isinstance(image, torch.Tensor) and image.dim() == 4 and image.size(0) == 1:
...
class PILResize:
    def __call__(self, image, target):
        ...
        dw /= 2
        dh /= 2
        top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1)) #107, 107
