...
from modules.block import (C2f, SPPF, Bottleneck, ViTEncoder, ViTEncoderLayer, Conv, Concat, Conv2, ConvTranspose, DWConvTranspose2d,
                     GhostConv, GhostBottleneck, SPP, DWConv, Focus, BottleneckCSP, C1, C2, C3, C3TR, C3Ghost, C3x, RepC3,
                     HGStem, HGBlock)
from modules.utils import extract_filename, LOGGER, make_divisible, non_max_suppression, scale_boxes
...
def yaml_load(file='kitti.yaml', append_filename=True):
    """
...
class YoloDetectionModel(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if isinstance(x, dict):# for cases of training and validating while training.
            ...
            preds = self._predict_once(x) #tensor input #[1, 3, 256, 256]
...
twoargs_blocks=[Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        #     args = [ch[f], *args]
            ...
            if m is HGBlock:
                ...
                n = 1
        elif m is ViTEncoder:
            c1, c2 = ch[f], args[0]
            if c2 != no:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # embed_dim, num_heads, n_encoder, mlp_ratio, dropout = args[:5]
            args = [c1, *args[1:]]
        # elif m is ViTEncoderLayer:
        #     embed_dim, num_heads, mlp_ratio, dropout, attn_dropout = args[:5]
        #     args = [ch[f], num_heads, mlp_ratio, dropout, attn_dropout]
        elif m is nn.BatchNorm2d:
            args = [ch[f]]
        elif m is Concat:
            c2 = sum(ch[x] for x in f)
        elif m in (Detect, Detect, Segment, Pose):
...
def create_yolomodel(modelname, num_classes = None, ckpt_file = None, fp16 = False, device = 'cuda:0', scale='n'):
    ...
    modelcfg_file=os.path.join('/data/cmpe258-sp24/017553289/cmpe249/2DObjectDetection/modules', modelname+'.yaml')
    cfgPath='/data/cmpe258-sp24/017553289/cmpe249/2DObjectDetection/modules/default.yaml'
    myyolo = None
    preprocess =None
    classesList = None
...
if __name__ == "__main__":
    ...
    print(os.getcwd())
    modelcfg_file='/data/cmpe258-sp24/017553289/cmpe249/2DObjectDetection/modules/yolov8.yaml'
