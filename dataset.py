...
import csv
from ObjectDetection2D.dataset_kitti import KittiDataset
...
import torch
import ObjectDetection2D.transforms as reference_transforms
...
def get_dataset(datasetname, is_train, is_val, args):
    if datasetname.lower() == 'kitti':
        ds, num_classes = get_kittidataset(is_train, is_val, args)
    # elif datasetname.lower() == 'coco':
...
class args:
    data_path = '/data/cmpe258-sp24/017553289/cmpe249/dataset/Kitti' #'/data/cmpe249-fa23/WaymoCOCO/' #'/data/cmpe249-fa23/coco/'
    # annotationfile = '/data/cmpe258-sp24/017553289/cmpe249/dataset/Kitti/train/labels/'
    weights = None
    test_only = True
    backend = 'PIL' #tensor
    use_v2 = False
...
if __name__ == "__main__":
    is_train =True
    is_val =False
    datasetname='kitti' #'waymococo' #'yolo'
