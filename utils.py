...
def mycollate_fn(batch: List[Tuple[torch.Tensor, Dict[str, Any]]]) -> Dict[str, Any]:
    """
    Custom collate function for object detection tasks.

    Args:
        batch (List[Tuple[torch.Tensor, Dict[str, Any]]]): 
            A list of tuples where each tuple contains an image tensor and a target dictionary.

    Returns:
        Dict[str, Any]: A dictionary containing batched images and targets.
    """
    ...
    # Concatenate bounding boxes and labels from all targets
    image_ids = []
    bboxes = []
    cls = []
    batch_idx = []
    ...
    for i, target in enumerate(targets):
        ...
        if target["boxes"].shape[0] > 0:
            batch_idx.append(torch.full((target["boxes"].shape[0],), i, dtype=torch.int64))
            image_ids.append(torch.full((target["boxes"].shape[0],), target['image_id'], dtype=torch.int64))

    if bboxes:
        bboxes = torch.cat(bboxes, dim=0)  # Shape: (total_boxes, 4)
        cls = torch.cat(cls, dim=0)        # Shape: (total_boxes,)
    else:
        bboxes = torch.zeros((0, 4), dtype=torch.float32)
        cls = torch.zeros((0,), dtype=torch.int64)

    if batch_idx:
        batch_idx = torch.cat(batch_idx, dim=0)  # Shape: (total_boxes,)
        image_ids = torch.cat(image_ids, dim=0)
    else:
        batch_idx = torch.tensor([], dtype=torch.int64)
        image_ids = torch.tensor([], dtype=torch.int64)
...
#     new_batch['img_id'] = torch.tensor(list(new_batch['image_id']))
#     # print(new_batch['img_id'])
#     new_batch['batch_idx'] = torch.tensor(list(new_batch['image_id']))
#     print(new_batch['img_id'])
